...
The system should keep all timing-critical decisions inside the firmware and use the 
browser only as a user interface. The browser may pause, render slowly, or lose focus; 
the ESP32 must continue to generate the selected switching pattern without depending on 
those events.
...
The switching waveform is described at command level by two synchronized vectors:
the duration of each segment, expressed in microseconds, and the segment mode. The
mode is a compact decimal encoding of the three logical switch variables. For
example, mode 5 corresponds to the binary state 101. This representation is useful
for browser commands because it is short and easy to serialize, but it is not the
form used by the real-time GPIO loop.

Figure \ref{fig:sw_signal_pipeline} summarizes the complete conversion path. A
browser command, either an explicit \verb|signal.set_pattern| vector or an
\verb|signal.set_alpha| request, is handled on the command side of the firmware.
There, the compact pattern is parsed or computed, written into the inactive
dataset, expanded into the individual logical switch arrays, and finally converted
into a precomputed sequence of GPIO operations. The signal loop therefore consumes
only ready-to-use \verb|SignalStep| entries, rather than decoding modes during
playback.

\begin{figure}[H]
	\centering
	\includegraphics[width=180mm]{figures/images/sw_signal_pipeline.pdf}
	\caption{Signal pattern pipeline from uploaded durations and modes to precomputed GPIO steps.}
	\label{fig:sw_signal_pipeline}
\end{figure}

The separation between active and inactive datasets is the mechanism that makes
this pipeline deterministic. New patterns are never written into the dataset that
Core 1 is currently executing. Instead, the command path selects the inactive
dataset, fills the raw duration and mode arrays, expands each mode into individual
switch states, and precomputes the masks required by the signal loop. Only after
the inactive dataset contains a complete pattern does the firmware publish it by
setting the update-pending flag.

\begin{listing}[H]
\caption{Updating the inactive signal buffer}
\label{lst:inactive-buffer}
\begin{minted}{cpp}
...
The running task checks this flag only between complete pattern cycles. As shown
in the timing portion of Fig. \ref{fig:sw_signal_pipeline}, a command may arrive
while cycle $N$ is being generated, but Core 1 continues using the current dataset
until that cycle ends. The pointer swap occurs only at the next cycle boundary, so
the generated waveform never combines part of the old pattern with part of the new
one.

\begin{listing}[H]
\caption{Cycle-boundary buffer swap}
\label{lst:buffer-swap}
\begin{minted}{cpp}
...
\begin{listing}[H]
\caption{Precomputed step used by the playback loop}
\label{lst:signal-step}
\begin{minted}{cpp}
struct SignalStep {
...
\begin{listing}[H]
\caption{Atomic GPIO transition with dead-time}
\label{lst:switching}
\begin{minted}{cpp}
if (step.clear_mask) {
...
\begin{listing}[H]
\caption{Runtime ADC calibration lookup}
\label{lst:calib}
\begin{minted}{cpp}
