...
- [x] **Update t.m** - `run_tests()` only calls 3 tests but `+Runner/+Tests/` contains 12+ files; expand or document subset
...
- [x] **Analyze lookup generation flow** - Compare `+Runner/+Demos/z_create_lookup.m` with embedded `helper_datasetter.cpp/.h`
