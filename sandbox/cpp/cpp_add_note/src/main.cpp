...
int main() {
    ...
    note_buffer_add_text("Hello, World!");
    note_buffer_print_buffer();

    // 3. Add a string that almost fills the buffer.
    note_buffer_add_text(" bla\n");
    std::string long_string(NOTE_BUFFER_MAX_SIZE - 20, 'X');
    note_buffer_add_text(long_string);
    note_buffer_print_buffer();
    
    // 4. Try to add a small string that will not fit. This should fail.
    note_buffer_add_text("This should not be added.");
    note_buffer_print_buffer();

    // 5. Try to add a different string after the buffer is full. This should also fail.
    note_buffer_add_text("Another try.");
    note_buffer_print_buffer();
