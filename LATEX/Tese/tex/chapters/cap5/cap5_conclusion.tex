...
To test the method, first, a double integrator system with fixed dynamics was implemented in session \ref{sec:cap4:double-integrator}
...
The application example in session \ref{sec:cap4:buck-boost-converter} introduces a system with changing dynamics at each switching time. 
A cyclic trajectory was calculated in session \ref{sec:cap2:buck-boost-converter} to use as an input parameter to implement the control method.
...
The multilevel converter in session \ref{sec:cap4:multilevel-converter} is a circuit that 
