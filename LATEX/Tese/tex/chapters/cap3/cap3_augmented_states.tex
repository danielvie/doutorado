...
\label{sec:cap3:augmented-states}

The formulation above assumes one decision variable for each prediction step. For dwell-time constraints or block-constant actuation, it is useful to work with a model that advances in blocks of samples. Our hypothesis is that the control input will remain constant for $N_d$ consecutive cycles. To 
...
Starting with the standard discrete-time linear prediction equation \refeq{eq:cap3:dynamic-equation}:
