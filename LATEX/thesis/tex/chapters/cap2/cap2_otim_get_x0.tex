We need to compute the initial condition for the system for each evaluation of the cost 
function so that we have a periodic trajectory, meaning that the final states of the trajectory
are the same as the initial condition.
...
\begin{align}
    ...
    \label{eq:cap2:x0-periodic-condition-system}
\end{align}
...
From \eqref{eq:cap2:x0-periodic-condition-system} we can establish the relation between \highlight{$x(t_N)$} and \highlight{$x(0)$}
...
We impose the periodicity condition \highlight{$x(t_N) = x(0)$}, resulting in the 
following expression for the initial state \highlight{$x(0)$}:
...
\begin{align}
    ...
    \label{eq:cap2:x0-inverse-matrix}
\end{align}
...
\noindent
assuming that the inverse indicated in \eqref{eq:cap2:x0-inverse-matrix} exists.