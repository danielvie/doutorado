...
| `fixed_beta_0_2_feasible_region_vertices.csv` | Same producer, fixed-factor comparison |
...
| `figures/conditioned_control_response.pdf` | `paper.export_figures`, auxiliary large-error response |
...
The same pipeline writes `../latex/metrics.tex`, `../latex/lyapunov_metrics.tex`, and four main-text figures under `../latex/figures/`: `reference_schedule_orbit.pdf`, `trajectory_comparison.pdf`, `conditioned_control_response.pdf`, and `open_loop_convergence.pdf`. The auxiliary `invariant_raw_action_region.pdf` remains generated but is not a main-text figure. These publication inputs remain tracked so LaTeX builds without MATLAB or Julia. Never edit their numerical values manually.
