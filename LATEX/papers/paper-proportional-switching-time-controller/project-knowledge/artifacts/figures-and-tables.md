...
Source of truth: `latex/article.tex`, `scripts/generate_results.m`, and `scripts/generate_feasible_regions.m`
...
The figure labels and captions are defined in `latex/article.tex`. `scripts/generate_feasible_regions.m` generates the region figure; `scripts/generate_results.m` generates the reference and auxiliary simulation figures.
...
Both tables are written directly in `latex/article.tex`. Their numerical values use generated macros from `results/metrics.tex` where applicable.
