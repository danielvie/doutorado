# Earlier Lyapunov teaching study

This bundle was previously `studies/lyapunov/`. The latest analysis is now
[../lyapunov/analysis.jl](../lyapunov/analysis.jl), documented in
[../lyapunov/README.md](../lyapunov/README.md). Run it with `task lyapunov` from
the paper root.

The description below records the earlier teaching workflow. Its
`paper_certificate.m` expects an older stored-certificate schema and is not a
supported reproduction path for the current paper. The introductory CVX files
mentioned below actually live in `../lyapunov_focus/`; they are not duplicated
here. General examples and document sources are retained for study.
