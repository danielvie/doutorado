...
We aim to determine the set \highlight{$\mathcal{D}$} of points \highlight{$x(0) \in \mathcal{X}$} for 
...
in the space \highlight{$\mathbb{R}^{(pN+n)}$} corresponding to the variables \highlight{$\mathbf{u}[k]$}, \highlight{$x(0)$}.
