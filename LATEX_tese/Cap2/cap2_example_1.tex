...
with the topology shown in Figure \refeq{cap2-circuit-ex-1}. 
...
by \highlight{$[0.25, 0.25]$}. The lower bounds are defined as \highlight{$[0.25, 0.25]$},
which means that each cycle period will last $0.25 \;\mathrm{s}$ and the other optimizer will generate
values that are always bigger than $0.25 \;\mathrm{s}$. For the maximum cycle period, we 
use the \highlight{$Ax \le b$} constraint from \highlight{FMINCON} where \highlight{$A$} 
is a row of ones \highlight{$[1, 1, ...]$} and \highlight{$b$} is \highlight{$T_{p, max}$} 
so that the sum of all switching periods is less than the maximum cycle period allowed.
