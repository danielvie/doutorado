...
\noindent
\begin{align}
    x(t_1) &= e^{A_{\sigma(0)} t_1} x(0) + 
            \int_{0}^{t_1}{e^{A_{\sigma(0)}(t_1 - \tau)} b_{\sigma(0)} d\tau} \nonumber \\
...
\purple{
    \begin{align}
        F_i = e^{A_{\sigma(i)}t(i+1)} 
    \end{align}
    \begin{align}
        g_i = \int_{t_i}^{t_{i+1}}{e^{A_{\sigma(i)}(t_{i+1} - \tau)} b_{\sigma(i)} d\tau}
    \end{align}
}

From \eqref{cap2-get-x0-multi-equations} we can establish the relation between $x(t_N)$ and $x(0)$
