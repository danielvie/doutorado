...
void ble_router(esp_ble_gatts_cb_param_t *param) {
    if (param->write.len > 0) {
        ...
        if (sscanf(msg_lower, "blink:%hu,%hu", &blink_d1, &blink_d2) == 2) {
            ...
            ble_router_blink();
        } else if (sscanf(msg_lower, "port:%hu,%hu", &blink_d1, &blink_d2) ==
                   2) {
        ...
        } else if (sscanf(msg_lower, "us_delay:%lu", &g_dead_time_cycles_up) ==
                   1) {
            g_dead_time_cycles_down = g_dead_time_cycles_up;
            ESP_LOGI(TAG, "Setting `g_dead_time_cycles_up= %d`",
                     g_dead_time_cycles_up);
            ESP_LOGI(TAG, "Setting `g_dead_time_cycles_down= %d`",
                     g_dead_time_cycles_down);
        } else if (sscanf(msg_lower, "us_delay_up:%lu",
                          &g_dead_time_cycles_up) == 1) {
            ESP_LOGI(TAG, "Setting `g_dead_time_cycles_up= %d`",
                     g_dead_time_cycles_up);
        } else if (sscanf(msg_lower, "us_delay_down:%lu",
                          &g_dead_time_cycles_down) == 1) {
            ESP_LOGI(TAG, "Setting `g_dead_time_cycles_down= %d`",
                     g_dead_time_cycles_down);
        } else if (sscanf(msg_lower, "an_monitor_ms:%lu",
                          &g_analog_monitor_period_ms) == 1) {
            ESP_LOGI(TAG, "Setting `g_analog_monitor_period_ms = %d`",
...
void ble_router_blink_nn(uint16_t blink_d1, uint16_t blink_d2) {
    ...
    if (blink_d1 > 0 && blink_d2 > 0) {
        ...
        ESP_LOGI(TAG, "Blink delays set to D1=%u ms, D2=%u ms", blink_d1,
                 blink_d2);
        blink_create_task();
    }
}
...
void ble_router_status(void) {
    ...
    auto is_valid = [](DataSet &ds_) {
        return matrix_isvalid(ds_.gain_k) ? std::string("valid")
...
static void profile_event_handler(esp_gatts_cb_event_t event,
    ...
    case ESP_GATTS_REG_EVT: {
        ...
        esp_ble_gatts_create_service(
            gatts_if, &gl_profile_tab[PROFILE_APP_ID].service_id, 5);
        break;
    }
    case ESP_GATTS_CREATE_EVT: {
        ESP_LOGI(TAG, "Service created, handle %d",
        ...
        esp_err_t rc = esp_ble_gatts_add_char(
            gl_profile_tab[PROFILE_APP_ID].service_handle,
            &gl_profile_tab[PROFILE_APP_ID].char_uuid,
            ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, property, NULL, NULL);
        if (rc) {
    ...
    case ESP_GATTS_ADD_CHAR_EVT: {
        ESP_LOGI(TAG, "Char added, attr_handle %d",
        ...
        esp_err_t rd = esp_ble_gatts_add_char_descr(
            gl_profile_tab[PROFILE_APP_ID].service_handle, &descr_uuid,
