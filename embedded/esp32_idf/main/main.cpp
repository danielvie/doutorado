...
void blink_create_task() {
    ...
    if (!blink_task_handle) {
        BaseType_t r = xTaskCreate(blink_task, "blink_task", 2048, NULL, 5,
                                   &blink_task_handle);
        if (r != pdPASS) {
...
esp_err_t app_init() {
    ...
    if (ret) {
        ESP_LOGE("INIT", "BLE controller init failed: %s",
                 esp_err_to_name(ret));
        return ret;
    }
...
extern "C" void app_main(void) {
    ...
    if (cpu_freq_mhz < 240) {
        ESP_LOGE(TAG, "WARNING: ESP32 is running SLOW (%lu MHz).",
                 cpu_freq_mhz);
        ESP_LOGE(TAG,
                 "Please run 'idf.py menuconfig' -> Component config -> ESP "
    ...
    // Create analog task on Core 0 with sufficient stack size for
    // helper::printf
