  ...
  return {
    ...
    status: packet.status
      ? {
          ...
          signal_edge_overhead_up_cycles: numberValue(
            packet.status.signalEdgeOverheadUpCycles,
          ),
          signal_edge_overhead_down_cycles: numberValue(
            packet.status.signalEdgeOverheadDownCycles,
          ),
          analog: packet.status.analog
            ? {
                ...
                measured_triples_per_second: numberValue(
                  packet.status.analog.measuredTriplesPerSecond,
                ),
                ...
                acquisition_mode: numberValue(
                  packet.status.analog.acquisitionMode,
                ),
                samples_read: numberValue(packet.status.analog.samplesRead),
                samples_rejected: numberValue(
                  packet.status.analog.samplesRejected,
                ),
                channel_order_anomalies: numberValue(
                  packet.status.analog.channelOrderAnomalies,
                ),
                partial_triples: numberValue(
                  packet.status.analog.partialTriples,
                ),
    ...
    analog_diagnostic_result: packet.analogDiagnosticResult
      ? {
          ...
          overflow_delta: numberValue(
            packet.analogDiagnosticResult.overflowDelta,
          ),
          ...
          latency_avg_us: numberValue(
            packet.analogDiagnosticResult.latencyAvgUs,
          ),
          latency_p95_us: numberValue(
            packet.analogDiagnosticResult.latencyP95Us,
          ),
          fault_code: numberValue(packet.analogDiagnosticResult.faultCode),
          timing_samples: numberValue(
            packet.analogDiagnosticResult.timingSamples,
          ),
          playback_avg_us: numberValue(
            packet.analogDiagnosticResult.playbackAvgUs,
          ),
          loop_us: numberValue(packet.analogDiagnosticResult.loopUs),
          overruns: numberValue(packet.analogDiagnosticResult.overruns),
          timing_faults: numberValue(
            packet.analogDiagnosticResult.timingFaults,
          ),
      ...
      ? {
          ...
          cases: (packet.analogConfigTestResult.cases ?? []).map(
            (item: {
              ...
              playback_avg_us: numberValue(item.playbackAvgUsX100) / 100,
              loop_us: numberValue(item.loopUsX100) / 100,
              overruns_delta: numberValue(item.overrunsDelta),
              timing_faults_delta: numberValue(item.timingFaultsDelta),
              dma_samples_delta: numberValue(item.dmaSamplesDelta),
              dma_anomalies_delta: numberValue(item.dmaAnomaliesDelta),
            }),
          ),
