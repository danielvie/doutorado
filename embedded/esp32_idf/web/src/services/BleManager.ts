    ...
    if (uint8Array[0] === 0x01) {
      try {
        ...
        if (packet.telemetry) {
          ...
          const active_set =
            formatEnum(getVal(s.active_set, decodeBleSignalSet)) || "NONE";
          ...
          const control_state = formatEnum(
            getVal(s.control_state, decodeBleControlState),
          );
          ...
          const statusLines = [
            ...
            `Edge comp up   : ${s.signal_edge_overhead_up_cycles} cycles`,
            `Edge comp down : ${s.signal_edge_overhead_down_cycles} cycles`,
          ...
          if (state !== "OTA_DOWNLOADING" || s.message) {
            useBleStore
              .getState()
              .addLog(
                `OTA [${state}]: ${s.progress_percent}% - ${s.message || ""}`,
              );
          }
          ...
          if (!t.valid && t.running) {
            ...
            const message =
              t.message || "No analog diagnostic test result is available";
            useBleStore.getState().addLog(message);
            useBleStore.getState().setLastStatusMessage(message);
            useBleStore.getState().setLastStatusCommand({
              name: "debug.test.an.result",
          ...
          if (!t.valid) {
            const message =
              t.message || "No analog config sweep result is available";
            useBleStore.getState().addLog(message);
            useBleStore.getState().setLastStatusMessage(message);
            useBleStore.getState().setLastStatusCommand({
          ...
          if (r.name === "debug.signal_timing" && r.json) {
            try {
              ...
              useBleStore
                .getState()
                .setLastStatusMessage(timingLines.join("\n"));
              useBleStore
                .getState()
                .setLastStatusCommand(
