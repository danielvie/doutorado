 ...
 *(thanks to Eckhard Arnold) \version 3.2 \date 2007-2017
 *
 ...
 *	p r i n t
 */
returnValue print(const real_t *const v, int_t n, const int_t *const V_idx,
                  const char *name) {
#ifndef __SUPPRESSANYOUTPUT__
 ...
 *	p r i n t
 */
returnValue print(const real_t *const M, int_t nrow, int_t ncol,
                  const char *name) {
#ifndef __SUPPRESSANYOUTPUT__
 ...
 */
returnValue print(const real_t *const M, int_t nrow, int_t ncol,
                  const int_t *const ROW_idx, const int_t *const COL_idx,
                  const char *name) {
#ifndef __SUPPRESSANYOUTPUT__
...
returnValue printCopyrightNotice() {
  ...
  // 	myPrintf( "\nqpOASES -- An Implementation of the Online Active Set
  // Strategy.\nCopyright (C) 2007-2017 by Hans Joachim Ferreau, Andreas
  // Potschka,\nChristian Kirches et al. All rights reserved.\n\nqpOASES is
  // distributed under the terms of the \nGNU Lesser General Public License 2.1
  // in the hope that it will be \nuseful, but WITHOUT ANY WARRANTY; without
  // even the implied warranty \nof MERCHANTABILITY or FITNESS FOR A PARTICULAR
  // PURPOSE. \nSee the GNU Lesser General Public License for more details.\n\n"
  // ); 	#endif /* __NO_COPYRIGHT__ */ 	#endif /* __DSPACE__ */ 	#endif /*
  // __XPCTARGET__ */ #endif /* __SUPPRESSANYOUTPUT__ */
  return SUCCESSFUL_RETURN;
}
...
returnValue readFromFile(real_t *data, int_t nrow, int_t ncol,
  ...
  if ((datafile = fopen(datafilename, "r")) == 0) {
    ...
    return getGlobalMessageHandler()->throwError(RET_UNABLE_TO_OPEN_FILE,
                                                 errstr, __FUNC__, __FILE__,
                                                 __LINE__, VS_VISIBLE);
  }
  ...
  for (i = 0; i < nrow; ++i) {
    for (j = 0; j < ncol; ++j) {
...
#endif /* __USE_SINGLE_PRECISION__ */
      {
        ...
        char errstr[MAX_STRING_LENGTH];
        snprintf(errstr, MAX_STRING_LENGTH, "(%s)", datafilename);
        return getGlobalMessageHandler()->throwError(RET_UNABLE_TO_READ_FILE,
                                                     errstr, __FUNC__, __FILE__,
                                                     __LINE__, VS_VISIBLE);
      }
 ...
 */
returnValue writeIntoMatFile(FILE *const matFile, const real_t *const data,
                             int_t nRows, int_t nCols, const char *name) {
  /*  Note, this code snippet has been inspired from the document
   *  "Matlab(R) MAT-file Format, R2013b" by MathWorks */

#ifndef __SUPPRESSANYOUTPUT__

  if ((matFile == 0) || (data == 0) || (nRows < 0) || (nCols < 0) ||
      (name == 0))
    return RET_INVALID_ARGUMENTS;
  ...
  var.numericFormat = 0000; /* IEEE Little Endian - reserved - double precision
                               (64 bits) - numeric full matrix */
 ...
 *	w r i t e I n t o M a t F i l e
 */
returnValue writeIntoMatFile(FILE *const matFile, const int_t *const data,
                             int_t nRows, int_t nCols, const char *name) {
  ...
  returnValue returnvalue =
      writeIntoMatFile(matFile, realData, nRows, nCols, name);
  delete[] realData;
...
real_t getCPUtime() {
  ...
  current_time =
      1.0 * (real_t)theclock.tv_sec + 1.0e-6 * (real_t)theclock.tv_usec;
#endif
 ...
 */
returnValue getKktViolation(int_t nV, int_t nC, const real_t *const H,
                            const real_t *const g, const real_t *const A,
                            ...
                            const real_t *const workingSetB,
                            const real_t *const workingSetC,
                            BooleanType hasIdentityHessian) {
  /* Tolerance for dual variables considered zero. */
  const real_t dualActiveTolerance = 1.0e3 * qpEPS;
 ...
 *	g e t K k t V i o l a t i o n
 */
returnValue getKktViolation(int_t nV, const real_t *const H,
                            const real_t *const g, const real_t *const lb,
                            const real_t *const ub, const real_t *const x,
                            const real_t *const y, real_t &stat, real_t &feas,
                            real_t &cmpl, const real_t *const workingSetB,
                            BooleanType hasIdentityHessian) {
  return getKktViolation(nV, 0, H, g, 0, lb, ub, 0, 0, x, y, stat, feas, cmpl,
...
int_t getSimpleStatus(returnValue returnvalue, BooleanType doPrintStatus) {
  ...
  if (doPrintStatus == BT_TRUE) {
    VisibilityStatus vsInfo =
        getGlobalMessageHandler()->getInfoVisibilityStatus();
 ...
 */
returnValue normaliseConstraints(int_t nV, int_t nC, real_t *A, real_t *lbA,
                                 real_t *ubA, int_t type) {
  int_t ii, jj;
  real_t curNorm;
 ...
 */
extern "C" void gdb_printmat(const char *fname, real_t *M, int_t n, int_t m,
                             int_t ldim) {
#ifndef __SUPPRESSANYOUTPUT__
