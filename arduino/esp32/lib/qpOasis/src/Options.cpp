...
returnValue Options::setToDefault( )
{
	...
	terminationTolerance          =  1.0e2 * qpEPS;
	boundTolerance                =  1.0e2 * qpEPS;
	#else
	terminationTolerance          =  5.0e6 * qpEPS;
	boundTolerance                =  1.0e6 * qpEPS;
	#endif
	...
	#ifdef __USE_SINGLE_PRECISION__
	epsFlipping                   =  5.0e1 * qpEPS;
	#else
	epsFlipping                   =  1.0e3 * qpEPS;
	#endif
	numRegularisationSteps        =  0;
	#ifdef __USE_SINGLE_PRECISION__
	epsRegularisation             =  2.0e1 * qpEPS;
	numRefinementSteps            =  2;
	#else
	epsRegularisation             =  1.0e3 * qpEPS;
	numRefinementSteps            =  1;
	#endif
	epsIterRef                    =  1.0e2 * qpEPS;
	#ifdef __USE_SINGLE_PRECISION__
	epsLITests                    =  5.0e1 * qpEPS;
	epsNZCTests                   =  1.0e2 * qpEPS;
	#else
	epsLITests                    =  1.0e5 * qpEPS;
	epsNZCTests                   =  3.0e3 * qpEPS;
	#endif
...
returnValue Options::setToMPC( )
{
	...
	#ifdef __USE_SINGLE_PRECISION__
	terminationTolerance          =  1.0e3 * qpEPS;
	#else
	terminationTolerance          =  1.0e9 * qpEPS;
	#endif
...
returnValue Options::ensureConsistency( )
{
    ...
    */
	...
	{
		epsRegularisation = qpEPS;
		needToAdjust = BT_TRUE;
	}

	if ( boundTolerance <= 0.0 )
	{
		boundTolerance = qpEPS;
		needToAdjust = BT_TRUE;
	}

	if ( boundRelaxation <= 0.0 )
	{
		boundRelaxation = qpEPS;
		needToAdjust = BT_TRUE;
	}
	...
	{
		initialFarBounds = boundRelaxation+qpEPS;
		needToAdjust = BT_TRUE;
	}
