...
*/
	...
	for( iter=nWSRperformed; iter<nWSR; ++iter )
	{
		...
		{
			if ( ( tau <= qpEPS ) && ( options.enableRamping == BT_TRUE ) )
				performRamping( );
			else
...
returnValue QProblem::backsolveT( const real_t* const b, BooleanType transposed, real_t* const a ) const
{
	...
	if ( transposed == BT_FALSE )
	{
		...
		for( i=0; i<nT; ++i )
		{
			...
			if ( getAbs( TT(i,sizeT-1-i) ) > qpEPS )
	...
	else
	{
		...
		for( i=0; i<nT; ++i )
		{
			...
			if ( getAbs( TT(nT-1-i,tcol+i) ) > qpEPS )
											...
											)
{
	...
	for ( i=0; i<nAC; ++i )
	{
		...
		if ( ( getAbs( delta_lbA[ii] ) > qpEPS ) || ( getAbs( delta_ubA[ii] ) > qpEPS ) )
		{
		  								...
		  								)
{
	...
	switch ( options.printLevel )
	{
		case PL_DEBUG_ITER:
			...
			/* complementarity */
			for (i = 0; i < nV; i++) if (y[i] > +qpEPS && getAbs((lb[i] - x[i])*y[i]) > bcmpl) bcmpl = getAbs((lb[i] - x[i])*y[i]);
			for (i = 0; i < nV; i++) if (y[i] < -qpEPS && getAbs((ub[i] - x[i])*y[i]) > bcmpl) bcmpl = getAbs((ub[i] - x[i])*y[i]);
			for (i = 0; i < nC; i++) if (y[nV+i] > +qpEPS && getAbs((lbA[i]-AX[i])*y[nV+i]) > ccmpl) ccmpl = getAbs((lbA[i]-AX[i])*y[nV+i]);
			for (i = 0; i < nC; i++) if (y[nV+i] < -qpEPS && getAbs((ubA[i]-AX[i])*y[nV+i]) > ccmpl) ccmpl = getAbs((ubA[i]-AX[i])*y[nV+i]);
